...
# Crear espectrograma Mel con 80 canales
def create_mel_spectrogram(audio_path, n_mels=70):  # Ajustado a 80
    try:
...
channel_adjust = nn.Conv1d(in_channels=512, out_channels=80, kernel_size=1)  # Ajustado a 80 canales
...
def train(tacotron2, waveglow, audio_paths, texts):
    ...
    for epoch in range(EPOCHS):
        for i in range(0, len(audio_paths), BATCH_SIZE):
            ...
            # Cargar audios y ajustar longitudes
            ...
            # Ajustar tamaño de lote si es necesario
            if audio.shape != batch_audio_tensor.shape:
                print(f"Ajuste del tamaño del tensor audio.")
                audio = F.interpolate(audio, size=(batch_audio_tensor.shape[-1],), mode="nearest")
